...
impl SqliteVectorStore {
    ...
    pub async fn search_by_embedding(
        ...
        let results = scored
            .into_iter()
            .map(|(score, id, content, metadata_str)| {
...
impl VectorStore for SqliteVectorStore {
    ...
    async fn search_by_embedding(
        ...
        let vector_results =
            SqliteVectorStore::search_by_embedding(self, collection, query_embedding, limit * 2)
                .await?;
        let vector_ranked: Vec<_> = vector_results
            .iter()
            .map(|r| {
                (
                    r.id.clone(),
...
fn hybrid_rrf(
    ...
    for (rank, (id, _, content, metadata)) in vector_ranked.iter().enumerate() {
        *scores.entry(id.clone()).or_default() += 1.0 / (k + rank as f64 + 1.0);
        data.entry(id.clone())
            .or_insert_with(|| (content.clone(), metadata.clone()));
    }
...
mod tests {
    ...
    async fn test_upsert_with_embeddings_and_search() {
        ...
        let docs = vec![
            Document {
                id: "d1".into(),
                content: "Prayer times".into(),
                metadata: HashMap::new(),
            },
            Document {
                id: "d2".into(),
                content: "Fasting rules".into(),
                metadata: HashMap::new(),
            },
        ];
        ...
        store
            .upsert_with_embeddings("test", &docs, &embeddings)
            .await
            .unwrap();

        // Query closer to d1
        let results = store
            .search_by_embedding("test", &[0.9, 0.1, 0.0], 2)
            .await
            .unwrap();
    ...
    async fn test_fts5_text_search_with_bm25() {
        ...
        let docs = vec![
            ...
            Document {
                id: "q3".into(),
                content: "The most merciful and compassionate".into(),
                metadata: HashMap::new(),
            },
        ];
        store.upsert("quran", &docs).await.unwrap();
    ...
    async fn test_hybrid_rrf_search() {
        ...
        let docs = vec![
            Document {
                id: "d1".into(),
                content: "prayer prayer prayer times".into(),
                metadata: HashMap::new(),
            },
            Document {
                id: "d2".into(),
                content: "Fasting rules during Ramadan".into(),
                metadata: HashMap::new(),
            },
            Document {
                id: "d3".into(),
                content: "Solat schedule Malaysia".into(),
                metadata: HashMap::new(),
            },
        ];
        let embeddings = vec![
        ...
        let results = store_trait
            .search_by_embedding("test", &[0.9, 0.1, 0.0], "prayer", 3)
            .await
            .unwrap();
        assert!(!results.is_empty());
