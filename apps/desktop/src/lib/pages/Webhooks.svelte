...
<div class="max-w-5xl">
	<PageHeader title="Webhooks" subtitle="{#if tab === 'endpoints'}{endpoints.length} endpoint{endpoints.length !== 1 ? 's' : ''} configured{:else}Incoming webhook delivery history{/if}">
		{#snippet action()}
