...
<div class="p-8 max-w-4xl">
	...
	<div class="flex items-center justify-between mb-6">
		<div>
			<h2 class="text-xl font-semibold text-gray-900 tracking-tight">Cron Jobs</h2>
			<p class="text-sm text-gray-500 mt-1">
				{#if tab === 'jobs'}
					{jobCount} job{jobCount !== 1 ? 's' : ''}{#if timezone} &middot; {timezone}{/if}
				{:else}
					Execution history (auto-refreshes)
				{/if}
			</p>
		</div>
	...
	{#if tab === 'jobs'}
		...
		{#if showForm}
			<div class="bg-white rounded-xl border border-gray-200 p-5 mb-6">
				<h3 class="text-sm font-medium text-gray-900 mb-4">{editingId ? 'Edit' : 'Add'} Cron Job</h3>
				...
				<div class="space-y-4">
					...
					<!-- Schedule -->
					<div>
						<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-2">Schedule</label>
							...
							{#each presets as preset}
								<button onclick={() => { schedulePreset = preset.value; onPresetChange(); }}
									class="px-3 py-1.5 text-xs font-medium rounded-md border transition-colors
										{schedulePreset === preset.value ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}">
									{preset.label}
								</button>
							{/each}
						</div>
						...
						<p class="text-[10px] text-gray-400 mt-1">min hour day month weekday</p>
					</div>

					<!-- Type -->
					<div>
						<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-2">Type</label>
						<div class="flex gap-2">
							<button onclick={() => jobType = 'direct_message'}
								class="px-3 py-1.5 text-xs font-medium rounded-md border transition-colors
									{jobType === 'direct_message' ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}">
								Direct Message
							</button>
							<button onclick={() => jobType = 'skill_invocation'}
								class="px-3 py-1.5 text-xs font-medium rounded-md border transition-colors
									{jobType === 'skill_invocation' ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}">
								Skill Invocation
							</button>
							<button onclick={() => jobType = 'agent_prompt'}
								class="px-3 py-1.5 text-xs font-medium rounded-md border transition-colors
									{jobType === 'agent_prompt' ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}">
								Agent Prompt
							</button>
						</div>
					</div>
					...
					{#if jobType === 'direct_message'}
						<div>
							<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">Message Template</label>
							<textarea bind:value={template} rows={3} placeholder="Message to send..."
								class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 resize-y"></textarea>
						</div>
					{:else if jobType === 'skill_invocation'}
						<div>
							<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">Skill Name</label>
							<input type="text" bind:value={skillName} placeholder="e.g. solat"
								class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900">
						</div>
						<div>
							<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">Input JSON</label>
					...
					{:else if jobType === 'agent_prompt'}
						<div>
							<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">Agent Name</label>
							<input type="text" bind:value={agentName} placeholder="e.g. default"
								class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900">
						</div>
						<div>
							<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">Prompt</label>
					...
					<div>
						...
						{#if targets.length === 0}
							<p class="text-xs text-gray-400">No targets. Click + Add to add a target.</p>
						{/if}
						{#each targets as target, i}
							<div class="flex gap-2 mb-2">
								...
								<input type="text" bind:value={target.chat_id} placeholder="chat_id"
									class="flex-1 px-2 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-900">
					...
					<div>
						<label class="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1">
							Agent Override <span class="normal-case text-gray-400">(optional)</span>
						</label>
					...
					<div class="flex items-center gap-2">
						...
						<span class="text-xs text-gray-600">{enabled ? 'Enabled' : 'Disabled'}</span>
					</div>
				</div>
		...
		{:else if jobs.length === 0 && !showForm}
			<div class="text-center py-16 bg-gray-50 rounded-xl border border-gray-200">
				<p class="text-sm text-gray-500 mb-1">No cron jobs configured</p>
				<p class="text-xs text-gray-400">Schedule recurring tasks for your bot</p>
			</div>
				...
				<table class="w-full text-xs">
					<thead>
						<tr class="border-b border-gray-100">
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Name</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Schedule</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Type</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Targets</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Enabled</th>
							<th class="text-right px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Actions</th>
						</tr>
					</thead>
					<tbody>
						{#each jobs as job}
							<tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
								<td class="px-4 py-2.5 text-sm font-medium text-gray-900">{job.name}</td>
								<td class="px-4 py-2.5 font-mono text-gray-600">{job.schedule}</td>
								<td class="px-4 py-2.5">
									<span class="inline-flex px-1.5 py-0.5 text-[10px] font-medium rounded
										{job.type === 'direct_message' ? 'bg-blue-100 text-blue-700' :
										 job.type === 'skill_invocation' ? 'bg-purple-100 text-purple-700' :
										 'bg-amber-100 text-amber-700'}">
										{job.type.replace('_', ' ')}
									</span>
								</td>
								<td class="px-4 py-2.5 text-gray-600">{(job.targets || []).length}</td>
								<td class="px-4 py-2.5">
									<button onclick={() => toggleJob(job)}
										class="relative inline-flex h-4 w-7 shrink-0 rounded-full border-2 border-transparent transition-colors cursor-pointer
											{job.enabled ? 'bg-gray-900' : 'bg-gray-300'}">
								...
								<td class="px-4 py-2.5 text-right">
									<button onclick={() => editJob(job)}
										class="text-xs text-gray-500 hover:text-gray-900 font-medium mr-2">Edit</button>
	...
	{:else}
		{#if history.length === 0}
			<div class="text-center py-16 bg-gray-50 rounded-xl border border-gray-200">
				<p class="text-sm text-gray-500">No execution history yet</p>
				<p class="text-xs text-gray-400 mt-1">History will appear here once jobs start running</p>
			</div>
		{:else}
			<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
				<table class="w-full text-xs">
					<thead>
						<tr class="border-b border-gray-100">
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Job</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Status</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Output</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Duration</th>
							<th class="text-left px-4 py-2.5 text-[10px] font-medium text-gray-400 uppercase">Executed At</th>
						</tr>
					</thead>
					<tbody>
						{#each history as entry}
							<tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
								<td class="px-4 py-2.5 text-sm font-medium text-gray-900">{entry.job_id}</td>
								<td class="px-4 py-2.5">
									<span class="inline-flex px-1.5 py-0.5 text-[10px] font-medium rounded
								...
								<td class="px-4 py-2.5 text-gray-600 font-mono max-w-xs truncate" title={entry.output}>
									{truncate(entry.output, 60)}
								</td>
								<td class="px-4 py-2.5 text-gray-600">
									{entry.duration_ms != null ? `${entry.duration_ms}ms` : '-'}
								</td>
								<td class="px-4 py-2.5 text-gray-500">{formatDate(entry.executed_at)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
